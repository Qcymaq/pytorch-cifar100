...
def train_model(model, train_loader, val_loader, num_classes, batch_size=64, learning_rate=0.01, num_epochs=50, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, f'mobilenetv2_epoch_{epoch + 1}_valacc_{val_accuracy:.2f}.pth')
...
if __name__ == '__main__':
    ...
    model = mobilenetv2()  # Initialize MobileNetV2 model

    # Modify the model's output layer to match the number of classes
    model.conv2 = nn.Conv2d(1280, num_classes, 1)  # Adjust the last layer for class count
